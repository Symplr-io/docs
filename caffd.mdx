...
If the rate limit is exceeded, subsequent imports may fail or return incomplete results until the limit resets.

When importing Figma projects that contain a large number of images, Free and View-only Figma accounts may experience missing images during import.

This issue is more likely to occur during subsequent imports of the same project due to Figma API throttling and asset retrieval limits.
